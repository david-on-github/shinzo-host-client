...
A hub outage degrades to the static lists. Flipping the default to on is a one-line change once (a) indexer policing exists and (b) DefraDB's peer-exchange behaviour is confirmed — if gossipsub exchanges peers, a bootstrap cap is only a soft limit and the real controls are signature verification and DefraDB-level peer allow-listing. The IP multiaddrs in `networks.go` remain a stopgap until DNS names exist for the seed indexers (see Future work).
...
5. **Auto-generated keyring secret** persisted in the data volume when `DEFRA_KEYRING_SECRET` is unset, removing the last required input.
